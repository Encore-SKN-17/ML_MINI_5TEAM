...
총 15개 칼럼, 13개의 독립변수로 구성함 
| 컬럼명            | 설명                                   |
|-------------------|--------------------------------------|
| `date`            | 날짜 (YYYY-MM-DD)                    |
| `ride_count`      | 지하철 승차 인원                       |
| `AvgTemp(°C)`     | 평균 기온 (℃)                         |
| `Rainfall(mm)`    | 강수량 (mm)                           |
| `WindSpeed(m/s)`  | 풍속 (m/s)                            |
| `Humidity(%)`     | 평균 습도 (%)                         |
| `isolation(MJ/m2)`| 일사량 (MJ/m²)                        |
| `holiday_flag`    | 주말/공휴일 여부 (1=주말/공휴일, 0=평일) |
| `미세 먼지 농도`   | 평균 미세먼지 농도 (㎍/m³)               |
| `PM10`            | PM10 농도 (㎍/m³)                     |
| `PM2.5`           | PM2.5 농도 (㎍/m³)                    |
| `오 존`           | 오존 농도 (ppm)                        |
| `이산화질소`       | 이산화질소 농도 (ppm)                   |
...
최종적 데이터 셋 **지하철 승차 인원 + 기상 데이터 + 대기오염 데이터 + 주말/공휴일 정보** 포함

| date       | ride_count | AvgTemp(°C) | Rainfall(mm) | WindSpeed(m/s) | Humidity(%) | isolation(MJ/m2) | holiday_flag | 미세 먼지 농도 | PM10 | PM2.5 | 오 존 | 이산화질소 | 일산화탄소 | 아황산가스 |
|------------|------------|-------------|--------------|----------------|-------------|-------------------|--------------|----------------|------|-------|-------|------------|------------|------------|
| 2019-01-01 | 39420.0    | -5.0        | 0.0          | 2.1            | 49.5        | 7.84              | 1            | 25.0           | 36.0 | 25.0  | 0.02  | 0.03       | 0.6        | 0.003      |
| 2019-01-01 | 11807.0    | -5.0        | 0.0          | 2.1            | 49.5        | 7.84              | 1            | 25.0           | 36.0 | 25.0  | 0.02  | 0.03       | 0.6        | 0.003      |
| 2019-01-01 | 20944.0    | -5.0        | 0.0          | 2.1            | 49.5        | 7.84              | 1            | 25.0           | 36.0 | 25.0  | 0.02  | 0.03       | 0.6        | 0.003      |
| 2019-01-01 | 17798.0    | -5.0        | 0.0          | 2.1            | 49.5        | 7.84              | 1            | 25.0           | 36.0 | 25.0  | 0.02  | 0.03       | 0.6        | 0.003      |
| 2019-01-01 | 13578.0    | -5.0        | 0.0          | 2.1            | 49.5        | 7.84              | 1            | 25.0           | 36.0 | 25.0  | 0.02  | 0.03       | 0.6        | 0.003      |
...
| 이재은 |이번 프로젝트에서는 경사하강법 기반의 모델, 선형회귀(Linear Regression), 서포트 벡터 회귀(SVR) 모델을 직접 구현하고 비교하는 작업을 맡았다. 각각의 모델을 구현하며 머신러닝 알고리즘이 작동하는 원리에 대해 더 깊이 이해할 수 있었고, 특히 경사하강법을 통해 직접 손실 함수의 최소점을 찾아가는 과정을 수치적으로 확인한 점이 인상 깊었다. 모델 성능이 낮게 나온 원인을 파악하기 위해 여러 접근을 시도했고, 그 과정에서 PolynomialFeatures를 적용해 비선형성을 부여하고, SVR의 C, epsilon, kernel 같은 하이퍼파라미터 튜닝을 통해 성능을 개선해보았다. | 
