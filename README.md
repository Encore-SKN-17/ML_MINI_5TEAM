...
| 이재은 |이번 프로젝트에서는 경사하강법 기반의 모델, 선형회귀(Linear Regression), 서포트 벡터 회귀(SVR) 모델을 직접 구현하고 비교하는 작업을 맡았다. 각각의 모델을 구현하며 머신러닝 알고리즘이 작동하는 원리에 대해 더 깊이 이해할 수 있었고, 특히 경사하강법을 통해 직접 손실 함수의 최소점을 찾아가는 과정을 수치적으로 확인한 점이 인상 깊었다. | 
